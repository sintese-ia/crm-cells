"use client";
import { useState, useTransition, useMemo, useImperativeHandle, forwardRef } from "react";
import { criarInteracao } from "@/app/actions/contas";
import { TIPOS_INTERACAO, type Contato, type Situacao } from "@/db/schema";
import { toast } from "sonner";

export type NovaInteracaoHandle = { open: (tipo?: string) => void };

export const NovaInteracaoRef = forwardRef<
  NovaInteracaoHandle,
  { contaId: number; contatos: Contato[]; situacoes: Situacao[] }
>(function NovaInteracaoRef({ contaId, contatos, situacoes }, ref) {
  ...
  return (
    ...
    >
      ...
      >
          ...
          >
            ...
            {Object.entries(situacoesPorEstagio).map(([estagio, sits]) => (
              <optgroup key={estagio} label={
                ...
                } as Record<string,string>)[estagio] ?? estagio
              }>
                {sits.map((s) => (
        ...
        <div>
          <label className="text-xs text-[#6B6B6B] uppercase tracking-wider">Data manual próxima ação (opcional)</label>
          <input type="date" name="dataPersonalizada" className="px-2 py-1.5 border border-[#E5E2DC] rounded text-sm bg-white" />
        </div>
        ...
        <div className="flex gap-2 justify-end pt-2 border-t border-[#E5E2DC]">
          ...
          <button type="submit" disabled={isPending || !situacaoId} className="text-sm px-4 py-1.5 rounded bg-[#0D0D0D] text-white disabled:opacity-50">
