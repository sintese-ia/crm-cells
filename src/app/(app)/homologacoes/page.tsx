...
export default async function HomologacoesPage() {
  ...
  const rows = await db.execute(sql`
    ...
    SELECT cr.rede,
           count(*)::int AS total,
           count(*) FILTER (WHERE cr.status_homologacao = 'pendente_inicio')::int AS pendente_inicio,
           count(*) FILTER (WHERE cr.status_homologacao = 'docs_enviados')::int AS docs_enviados,
           count(*) FILTER (WHERE cr.status_homologacao = 'em_analise')::int AS em_analise,
           count(*) FILTER (WHERE cr.status_homologacao = 'aprovada')::int AS aprovada,
           count(*) FILTER (WHERE cr.status_homologacao = 'reprovada')::int AS reprovada,
           count(*) FILTER (WHERE cr.responsavel IS NULL)::int AS contas_sem_resp,
